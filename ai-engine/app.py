...
{{
  ...
  "mermaidDiagram": "graph TD\\n  A[\\\"Entry Point\\\"] --> B[\\\"Module\\\"]"
    ...
    response_format={"type": "json_object"}
      ...
      if "fileReviews" in result and isinstance(result["fileReviews"], dict):
        for _file, review in result["fileReviews"].items():
          ...
          for category in ("bugs", "security", "optimization", "styling"):
            ...
            for item in items:
              if isinstance(item, dict) and "suggestion" in item:
                item["suggestion"] = sanitize_ai_output(str(item["suggestion"]))
              if isinstance(item, dict) and "description" in item:
                item["description"] = sanitize_ai_output(str(item["description"]))
      return result
